...
#include <cuda.h>
#include "..\cutil\include\cutil_inline.h"
#include <stdio.h>
#include <stdlib.h>
...
void CullumDevice(int i, float* aAlpha, float*aBeta, double* tempAlpha, double* tempBeta, int eigCheck, float* aTEigVals, float* aaTEigVecs, int p_nEigNum, float* p_eigenValues, double *tvectors, char range, char order, double vl, double vu, int il, int iu, double abstol, int nsplit, double* w, int *iblock, int* isplit, double* work, int* iwork, int* ifail )
{
        int m, info;
	int tempn = i;
			...
			assert(w!=NULL);
			LAPACK_dstein(&tempn, tempAlpha, tempBeta, &getNEig, w, iblock, isplit, tvectors, &tempn, work, iwork, ifail, &info);
             ...
             int p_nEigNum, float* p_eigenValues, float* devEigVectors, int p_nOrthoChoice, float* devRSqrtSum, float p_fTolerance)
{
	...
	for (int iter = 0; iter < MAXITER; iter++)
	{
		...
		if (((i+1) % CHECKITER == 0) || (i == MAXITER - 1) || (i == maxIterationsThatFitGPU-2))
		{
                        ...
                        CullumDevice(i, aAlpha, aBeta, tempAlpha, tempBeta, eigCheck, aTEigVals, aaTEigVecs, p_nEigNum, p_eigenValues, tvectors, range, order, vl,  vu, il, iu, abstol, nsplit, w, iblock, isplit, work, iwork, ifail );
