...
//////////////////////////////////////////////////////////////////////////
/// parabola_kernel
__global__ void parabolaKernel(float* trace, int width, int height, int limit, int border, int border_height, int filter_radius, int filter_length, int filter_width)
{
...
static inline void copy_cuda_parabola_buffers(int norients, int width, int height, int border, float *devPixels, int filter_radius, int filter_length, float* host_filters)
{
    ...
    // copy pixels
	CUDA_SAFE_CALL(	cudaMemcpy2DToArray(cuda_parabola_pixels, 0, 0, devPixels, border_width*sizeof(int), 
		border_width*sizeof(int), border_height*norients, cudaMemcpyDeviceToDevice));
