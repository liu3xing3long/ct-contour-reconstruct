...
__global__ void computeGradient(int width, int height, int nPixels, int border, int rotatedWidth, 
								float aNorm, float bNorm, int kernelRadius, int kernelLength, int* devIntegrals, 
								int integralImagePitch, float* devGradientA) 
{
	...
	//printf("threadIdx.x:%d, threadIdx.y:%d, blockIdx.x:%d, blockIdx.y:%d, internal bin:%d \n", threadIdx.x, threadIdx.y, blockIdx.x, blockIdx.y, bin);
	for(int y = threadIdx.y; y < height; y += UNROLL) 
	{
		...
		*/
...
#else
			int iDx = bin + blockDim.x;
			if (iDx <nthreads * UNROLL){
			}
			else{
				iDx = iDx%(nthreads * UNROLL);
			}
			temp[iDx] = 0;
#endif
		}
		...
		/**
		* 计算x2距离
		* x2(d) = [(aHistogram[bin] - bHistogram[bin])2]/[aHistogram[bin] + bHistogram[bin]]
		*
		*/
...
*/
void dispatchGradient(bool tg, int width, int height, int border, int nbins, 
					  float thetaPi, int rotatedWidth, int radius, bool blur, float sigma, 
					  int* devIntegrals, int integralImagePitchInInts, float* devGradientA, float* devGradientB) 
{
	float theta = float(thetaPi * M_PIl);

	//float effectiveRadius = M_SQRT2l*(-4.0+4.0*float(radius)*sqrtf(M_PIl))/8.0;
	float effectiveRadius = float(-4.0+4.0*float(radius)*sqrtl(M_PIl))/8.0f;
	...
	if (!tg) {
		...
		computeGradient<48, 25, true, false><<<gridDim, blockDim>>>(
			width, height, width * height, border, rotatedWidth, 
			topNorm, bottomNorm, kernelRadius, kernelLength, devIntegrals, 
			integralImagePitchInInts, devGradientA);
		computeGradient<48, 25, true, true><<<gridDim, blockDim>>>(
			width, height, width * height, border, rotatedWidth, 
			leftNorm, rightNorm, kernelRadius, kernelLength, devIntegrals, 
			integralImagePitchInInts, devGradientB);
...
void rotateImage(int width, int height, int* devImage, float thetaPi, int& newWidth, int& newHeight, int* devOutput) {
	...
	if (thetaPi == 0) {
		...
		newWidth = int(ceilf(width - (height - 1) * tanTheta));
		newHeight = int(ceilf(height - (width - 1) * tanTheta));
		int length;
		int yDescent = int(-round(((float)(width - 1)) * tanTheta));
		int xTravel = int(-round(((float)(height - 1)) * tanTheta));
		...
		float theta = float((thetaPi + 0.25) * M_PIl);
		float tanTheta = tanf(theta);
		newWidth = int(ceilf(height - (width - 1) * tanTheta));
		newHeight = int(ceilf(width - (height - 1) * tanTheta));
		int length;
		int xTravel = int(-round(((float)(width - 1)) * tanTheta));
		int yTravel = int(-round(((float)(height - 1)) * tanTheta));
