...
#ifndef DATA_PROFILE
 	const int szPIC_SIZE  = 75;
 	char* dirName = argv[1];
 	for (int iPicIdx = 0; iPicIdx < szPIC_SIZE; iPicIdx++)
#endif
	{
#ifndef DATA_PROFILE
		char filename[MAX_PATH];
		sprintf(filename, "%s\\%d%s", dirName, iPicIdx, ".ppm");
#else
		...
		char outputPGMfilename[MAX_PATH];
		char outputthinPGMfilename[MAX_PATH];
		char outputPBfilename[MAX_PATH];
		char outputthinPBfilename[MAX_PATH];
...
#ifdef DATA_PROFILE
		strncpy(outputgpbTextfilename, filename, period - filename);
		sprintf(&outputgpbTextfilename[0] + (period - filename), "_text.txt");
