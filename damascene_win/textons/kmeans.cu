...
void findSgemmLabels(float* devA, int nA, int aPitchInFloats, float* devB, int nB, int bPitchInFloats, int nDimension, float* devADots, float* devBDots, float* devDiff, int diffPitchInFloats, int* devClusters, int* devChanges) {
  findDiffNorm(devA, nA, aPitchInFloats, devB, nB, 
				bPitchInFloats, nDimension, devADots, devBDots, devDiff, 
				diffPitchInFloats);
...
// yunsup fixed
  ...
  for(i = 0; i < maxIter; i++) {
	 ...
	 */
    findSgemmLabels(
					devResponses, nPixels, nPixels, devCentroids, clusterCount, 
					clusterCount, filterCount, devPointsDots, devCentroidsDots, devDist, 
					devDistPitchInFloats, devClusters, devChanges);
