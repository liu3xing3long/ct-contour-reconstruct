...
//   }
   ...
   std::complex<float>*dft = new std::complex<float>[size];
   std::complex<float>*idft = new std::complex<float>[size];
...
//    *filters = (float*)realloc(*filters, 17*filterLength*sizeof(float));
        ...
        gaussian_cs_2D(*filters+i, scale[n],scale[n],0, /*M_SQRT2l*/M_SQRT2, support, support);
